...
def get_stock_data(code, days=90):
    ...
    # 한국 주식은 종목코드에 .KS (코스피) 또는 .KQ (코스닥)를 붙여야 합니다.
    # 사용자가 코드를 실수로 숫자만 입력했을 경우를 대비해 처리 로직을 추가합니다.
    ...
    try:
        ...
        # 주식시장 마감 시간 직후 등, 간혹 데이터가 없을 수 있으므로 try/except로 처리합니다.
        ...
        return pd.DataFrame(), f"❌ 데이터를 가져올 수 없습니다. 코드 {code}를 확인하세요."
...
with col1:
    ...
    if st.button("주가 검색 및 분석"):
        ...
        if not df_stock.empty:
            ...
            st.metric(label="최근 종가", value=f"{df_stock['종가'].iloc[-1]:,}", delta=f"{df_stock['변화율 (%)'].iloc[-1]:.2f}%")
            
            st.info(f"기간 내 최고 종가: {max_